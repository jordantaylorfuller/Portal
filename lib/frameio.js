...
// Find the first immediate child folder of `parentFolderId` whose `name`
// matches exactly. Used to locate convention-named subfolders like
// "_DELIVERABLES" inside a project's root folder. Returns null if no match.
async function findChildFolderByName(accountId, parentFolderId, name) {
  const children = await listFolderChildren(accountId, parentFolderId);
  for (const child of children) {
    const type = child.type || (child._type && child._type.toLowerCase());
