...
async function exchangeCode({ code, redirectUri }) {
  ...
  // Pull /v4/me to capture account_id + email so the admin UI can display it.
  let me = null;
  try {
    me = await rawApi('/me', { accessToken: json.access_token });
  } catch (err) {
    console.error('Frame.io /me lookup after exchange failed:', err.message);
  }
  const meData = me && me.data ? me.data : {};
  ...
  await adminClient.from('frameio_tokens').upsert({
    ...
    account_id: meData.account_id || null,
